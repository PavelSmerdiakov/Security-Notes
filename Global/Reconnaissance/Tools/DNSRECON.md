...
Outils pour l'énumération, DNS recon et information gathering.

- Enumeration de subdomaines
- Bruteforce les subdomains d'un site.
- Récupération de données DNS
- Zone transfert
- Reverse DNS lookup


```
usage: dnsrecon [-h] [-d DOMAIN] [-n NS_SERVER] [-r RANGE] [-D DICTIONARY] [-f] [-a] [-s] [-b] [-y] [-k] [-w]
                [-z] [--threads THREADS] [--lifetime LIFETIME] [--tcp] [--db DB] [-x XML] [-c CSV] [-j JSON]
                [--iw] [--disable_check_recursion] [--disable_check_bindversion] [-V] [-v] [-t TYPE]
...
```
https://www.thehacker.recipes/web/recon/domains-enumeration

###### - Enumeration de subdomaines

- Pour avoir des infos général DNS :
